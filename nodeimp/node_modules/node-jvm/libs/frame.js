...
Native["java/lang/Object.hashCode.()I"] = function() {
    ...
    for(let i=0;i<this.toString().length;i++){
        ...
        hashcode = hashcode*31+this.toString().charCodeAt(i)
        hashcode &= 0xffffffff
    }
...
Native["java/lang/Class.forName0.(Ljava/lang/String;)V"] = function(className) {
    var classInfo = null;
    className=className.replaceAll('.','/')  
    classInfo = global['CLASSES'].getClass(className);  
};

Native["java/lang/Class.forName1.(Ljava/lang/String;)Ljava/lang/Class;"] = function(className) {
    var classInfo = null;
    className=className.replaceAll('.','/') 
    classInfo = global['CLASSES'].getClass(className); 
    return classInfo;
...
Frame.prototype._throw = function(ex) { 
    ...
    if (handler_pc != null) {
        ...
        //console.error(ex);
        console.error("throw ex : "+ex.getClassName());
...
function findNativeMethod(className,methodName,signaturename){
    var sign = className+'.'+methodName+'.'+signaturename;
    //console.log('sign:',sign)
    var ret = Native[sign];
    if(ret){
        console.info('native function : ',sign,' found successed!');
    }
    return ret;
}

function nativeMethod(frame,args,className,methodName,signature){

    //console.log(JSON.stringify(frame))
    if( frame && frame.mymethod && ACCESS_FLAGS.isNative(frame.mymethod.access_flags) )
    {  
        var signaturename=frame._cp[frame.mymethod.signature_index].bytes;
 
        var nativeMethod=findNativeMethod(className,methodName,signaturename);
        
        if(nativeMethod)
        {
            var res = nativeMethod.apply(null, args);
            ...
            console.warn('native Method ',className+'.'+methodName+'.'+signaturename,' not implemented');
            return 1;
        }
    } 
    return 0;
...
Frame.prototype.invokestatic = function(done) {
    ...
    if(nativeMethod(method,args,className,methodName,signature))
    {
        return done();
    }
...
Frame.prototype.invokespecial = function(done) {
    ...
    if(nativeMethod(ctor,args,className,methodName,signature))
    {
        return done();
    }
...
Frame.prototype.invokeinterface = function(done) {
    ...
    if(nativeMethod(instance[methodName],args,className,methodName,signature))
    {
        return done();
    } 
