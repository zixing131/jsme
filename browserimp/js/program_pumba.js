...
js2me.generateProgram = function (data) {
	...
	generators[0xb2] = function () {
		...
		return generateLoadClass(field.className, function (context, classObj) {
			...
			}else if(js2me.statics1)
			{
				context.stack.push(js2me.statics1['$' + fieldId]); 
			}
		});
	};
	...
	{
		var argumentsTypes=methodType.argumentsTypes;
		...
		for(var i=0;i<argumentsTypes.length;i++){
			name+=argumentsTypes[i];
		}
		name+=')';
		name+=returnType;
		console.log("deName "+name);  
		return name;
	}
				...
				if (isVirtual) {
					try {
						try{
							...
							name  = deName(name,methodInfo.type);
							obj = !(obj[name]) && obj.as ? obj.as(methodInfo.className) : obj;
							result = obj[name].apply(obj, args);
							}catch(err){
								var name = methodInfo.name;
								name  = enFuncName(name,methodInfo.type);
								obj = !(obj[name]) && obj.as ? obj.as(methodInfo.className) : obj;
								result = obj[name].apply(obj, args);
							}
						}
						...
						if (isStatic) {
							...
							}catch(err)
							{
								try{
									...
									result = classObj.prototype[name].apply(classObj.prototype, args);
								}catch(err)
								{
									var name = methodInfo.name;
							...
							}catch(err)
							{
								...
								var name = methodInfo.name;
								name  = deName(name,methodInfo.type);
								result = classObj.prototype[name].apply(obj, args);
							}catch(err)
							{
								var name = methodInfo.name;
								name  = enFuncName(name,methodInfo.type);
