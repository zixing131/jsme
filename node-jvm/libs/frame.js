...
Frame.prototype._throw = function(ex) { 
    ...
    if (handler_pc != null) {
        ...
        console.error(ex);
        console.error(ex.getClassName());
        //this would be removed -- by zixing
       // throw ex;
    }
}
        ...
        SCHEDULER.tick(self._pid, function() {
            ...
            switch (opCode) {
                ...
                default:
                    ...
                    if (!(opName in self)) {
                        console.error(util.format("Opcode %s [%s] is not supported.", opName, opCode)); 
                        return //done();
...
//             this._stack.push(constant.bytes);
//             break;
//         default:
//             throw new Error("not support constant type tag:"+constant.tag);
//     }
//     return done();
// }
...
Frame.prototype.ldc = function(done) {
    ...
    switch(constant.tag) {
        ...
        case TAGS.CONSTANT_Class:
            ...
            this._stack.push(this._cp[constant.name_index].bytes);
            break;
            
        case TAGS.CONSTANT_MethodType:
            // Push method type reference onto the stack
            this._stack.push(this._cp[constant.descriptor_index].bytes);
            break;
            
        case TAGS.CONSTANT_MethodHandle:
            // Push method handle reference onto the stack
            this._stack.push({
                reference_kind: constant.reference_kind,
                reference_index: constant.reference_index
            });
            break;
            
        default:
            throw new Error("Unsupported constant type tag: " + constant.tag);
    }
    return done();
};
...
Frame.prototype.ldc_w = function(done) {
    ...
    switch(constant.tag) {
        case TAGS.CONSTANT_String:                        
            ...
            break;
        case TAGS.CONSTANT_Class:
            this._stack.push(constant.name_index);
            break;
        default:
            throw new Error("not support constant type "+constant.tag);
    }
    return done();
}
...
Frame.prototype.arraylength = function(done) {
    ...
    if(ref){ //this is modify by zixing
...
Frame.prototype.invokevirtual = function(done) {
    ...
    LOG.debug("CLASSES.getMethod className=" +  className +',methodName='+ methodName+ ' signature='+ signature);   
